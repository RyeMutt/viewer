...
LLRenderTarget& LLSceneMonitor::getCaptureTarget()
{
    ...
    if(!mFrames[0])
    {
        ...
        LLRender::instance().getTexUnit(0)->bind(mFrames[0]);
    ...
    else if(!mFrames[1])
    {
        ...
        LLRender::instance().getTexUnit(0)->bind(mFrames[1]);
        LLRender::instance().getTexUnit(0)->setTextureFilteringOption(LLTexUnit::TFO_POINT);
        LLRender::instance().getTexUnit(0)->unbind(LLTexUnit::TT_TEXTURE);
...
void LLSceneMonitor::capture()
{
        ...
        && last_capture_frame != gFrameCount)
    {
        ...
        LLRender::instance().getTexUnit(0)->bind(&cur_target);
...
void LLSceneMonitor::compare()
{
    ...
    LLRender::instance().getTexUnit(1)->activate();
    LLRender::instance().getTexUnit(1)->enable(LLTexUnit::TT_TEXTURE);
    LLRender::instance().getTexUnit(1)->bind(mFrames[1]);
    LLRender::instance().getTexUnit(1)->activate();

    LLRender::instance().getTexUnit(2)->activate();
    LLRender::instance().getTexUnit(2)->enable(LLTexUnit::TT_TEXTURE);
    LLRender::instance().getTexUnit(2)->bind(mDitheringTexture);
    LLRender::instance().getTexUnit(2)->activate();
    ...
    LLRender::instance().getTexUnit(2)->disable();
    LLRender::instance().getTexUnit(2)->unbind(LLTexUnit::TT_TEXTURE);
