...
void LLFace::renderSelected(LLViewerTexture *imagep, const LLColor4& color)
{
    ...
    {
        LLRender::instance().getTexUnit(0)->bind(imagep);

        LLRender::instance().pushMatrix();
        if (mDrawablep->isActive())
        {
            LLRender::instance().multMatrix((GLfloat*)mDrawablep->getRenderMatrix().mMatrix);
        }
        else
        {
            LLRender::instance().multMatrix((GLfloat*)mDrawablep->getRegion()->mRenderMatrix.mMatrix);
        }
...
void LLFace::renderOneWireframe(const LLColor4 &color, F32 fogCfx, bool wireframe_selection, bool bRenderHiddenSelections, bool shader)
{
    if (bRenderHiddenSelections)
    {
        ...
        if (shader)
        {
            LLRender::instance().diffuseColor4f(color.mV[VRED], color.mV[VGREEN], color.mV[VBLUE], 0.4f);
    ...
    LLRender::instance().diffuseColor4f(color.mV[VRED] * 2, color.mV[VGREEN] * 2, color.mV[VBLUE] * 2, color.mV[VALPHA]);
